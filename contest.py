...
@pytest.fixture
def append_stat(request):
    def append(step_name):
        file_stats = os.stat("/path/to/your/config/file")
        file_size = file_stats.st_size
        file_mtime = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(file_stats.st_mtime))

        with open("/proc/loadavg", "r") as loadavg_file:
            loadavg_data = loadavg_file.read()

        with open(file_stats, "a") as stat_file:
            stat_file.write(f"{step_name}: {time.strftime('%Y-%m-%d %H:%M:%S')}, "
                            f"{file_size} bytes, "
                            f"Last modified: {file_mtime}, "
                            f"Load Average: {loadavg_data}\n")

    request.addfinalizer(append)
    return append